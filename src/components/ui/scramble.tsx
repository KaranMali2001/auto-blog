...
const ScrambleHover: React.FC<ScrambleHoverProps> = ({
  ...
  return (
    <motion.span onHoverStart={() => setIsHovering?.(true)} onHoverEnd={() => setIsHovering?.(false)} className={cn("inline-block whitespace-pre-wrap", className)} {...props}>
      <span className="sr-only">{displayText}</span>
      <span aria-hidden="true">
        {displayText.split("").map((char, index) => (
          <span key={index} className={cn(revealedIndices.has(index) || !isScrambling || !isHovering ? className : scrambledClassName)}>
            {char}
          </span>
        ))}
      </span>
