...
export function MasonryView({ commit, repository, selected = false, onSelect, onClick }: MasonryViewProps) {
  ...
  return (
    <div
      className={cn(
        ...
        selected
          ? "border-primary/60 from-primary/10 to-primary/5"
          : "border-border/50 from-card to-card/50 hover:border-primary/30",
        "overflow-hidden"
      )}
    ...
    >
      ...
      <div className="p-5 space-y-3 h-full flex flex-col">
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2">
            <div data-checkbox onClick={(e) => e.stopPropagation()}>
              <Checkbox
                checked={selected}
                onCheckedChange={() => onSelect?.(commit._id)}
              />
            </div>
          ...
          {hasBeenUsed && (
            <div className="rounded-full bg-green-500/20 px-2 py-0.5 text-xs font-medium text-green-700 dark:text-green-300">
              Used
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-primary">{repository.name}</span>
        </div>
      ...
      {/* Decorative Element */}
      <div className={cn(
        "absolute -right-8 -bottom-8 h-24 w-24 rounded-full opacity-10 transition-opacity",
        selected ? "bg-primary opacity-20" : "bg-muted group-hover:opacity-20"
      )} />
    </div>
  );
}
