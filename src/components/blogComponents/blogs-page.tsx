...
import { Calendar, ChevronLeft, ChevronRight, FileText, Trash2 } from "lucide-react";
...
export function BlogsPage() {
  ...
  const {
    ...
    isPending,
    error,
  } = useQueryWithStatus(api.schema.blog.getBlogs, {
    paginationOpts: {
      cursor: cursor,
      numItems: 9,
    },
  ...
  useEffect(() => {
    if (cursor !== null && !isPending && blogsData?.page) {
      window.scrollTo({
        top: 0,
        behavior: "smooth",
      });
  ...
  }, [cursor, blogsData?.page, isPending]);
  ...
  const handleNext = () => {
    if (blogsData && !blogsData.isDone && blogsData.continueCursor) {
      setIsTransitioning(true);
      setPreviousCursors([...previousCursors, cursor || ""]);
      setCursor(blogsData.continueCursor);
    }
  };
  ...
  const hasNext = blogsData && !blogsData.isDone;
    ...
    <div className="container mx-auto max-w-7xl space-y-8 px-4 py-8">
      ...
      {(hasNext || hasPrevious) && (
        <div className="flex items-center justify-center gap-2 mt-8">
          <Button variant="outline" size="sm" onClick={handlePrevious} disabled={!hasPrevious || isPending || isTransitioning} className="transition-all duration-200 disabled:opacity-50">
            <ChevronLeft className="h-4 w-4 mr-1" />
            Previous
          </Button>
          <Button variant="outline" size="sm" onClick={handleNext} disabled={!hasNext || isPending || isTransitioning} className="transition-all duration-200 disabled:opacity-50">
            Next
